...
数論、modint、素数判定、大整数素因数分解、離散対数を提供します。

| Header | Main APIs |
| --- | --- |
| `math.hpp` | `fast_pow(a, n)`, `mod_pow(a, n, mod)` |
...
`modint` は `pow`, `inv`, `val`, 四則演算、入出力に対応します。64-bit 数論では `__int128` を使います。`factorize_pollard` は素因数を重複込み昇順で返します。

## Linear Algebra / Polynomial / Bit Transform

線形代数、畳み込み、FPS、線形漸化式、bit DP 変換を提供します。

| Header | Main APIs |
| --- | --- |
| `matrix.hpp` | `Matrix<T>`, `identity_matrix`, `matrix_mul`, `matrix_pow` |
| `linear_algebra.hpp` | `gaussian_elimination`, `matrix_rank`, `solve_linear_system` |
| `xor_basis.hpp` | `XorBasis::insert`, `can_make`, `max_xor`, `rank` |
| `convolution.hpp` | `convolution_ntt<Mint>`, `convolution_fft` |
| `fps.hpp` | `FormalPowerSeries<Mint>` の `diff`, `integral`, `inv`, `+`, `-`, `*` |
| `linear_recurrence.hpp` | Kitamasa `linear_recurrence_nth(init, coef, n)` |
| `berlekamp_massey.hpp` | `berlekamp_massey(sequence)` |
| `subset_transform.hpp` | `subset_zeta`, `subset_mobius`, `superset_zeta`, `superset_mobius` |
| `fwht.hpp` | `fwht_xor`, `convolution_xor`, `convolution_or`, `convolution_and` |

`linear_algebra.hpp` は `modint` のような体上の型を主対象にします。`convolution_ntt` と `FormalPowerSeries` は NTT-friendly な `static_modint<MOD>` を前提にします。`FFT convolution` は `ll` 配列向けで、浮動小数誤差に注意します。
...
| Name | Specification |
| --- | --- |
| `GraphDFS` | `used`, `parent`, `tin`, `tout`, `comp_id`, `order`, `post_order`, `comp_cnt`, `timer` を持つ DFS 結果 |
| `GraphDFS(g)` | 全頂点を DFS する |
| `GraphDFS(g, s)` | 単一起点 DFS を行う |
| `GraphDFS::build(g, s)` | 単一起点 DFS を行い直す |
| `GraphDFS::build_multi(g, starts)` | 複数始点 DFS を行う。未到達頂点は探索しない |
| `GraphDFS::build_all(g)` | 全頂点を DFS し直す |
| `GraphDFS::build_all(g, starts)` | `starts` の順に DFS した後、残りの頂点を昇順に DFS する |
...
`GraphDFS` と `ConnectedComponents` は有向グラフでも隣接リストに従って探索します。無向連結成分として使う場合は `undirected = true` の `Graph` を使います。`GraphDFS::order` は行きがけ順、`GraphDFS::post_order` は帰りがけ順で、どちらも内部 0-indexed の頂点番号を保持します。`GraphBFS` の未到達距離は `-1` です。
...
| `DirectedCycle` | `cycle`, `has_cycle` を持つ有向閉路検出結果 |
| `DirectedCycle(g)` | DFS で有向閉路を 1 つ検出する |
| `DirectedCycle::build(g)` | 有向閉路検出を行い直す |
| `DirectedCycle::cycle_external()` | `Graph::one_indexed` に従う外部 index の閉路を返す |
| `SCC` | `comp_cnt`, `comp_id`, `groups` を持つ強連結成分分解 |
| `SCC(g)` | Kosaraju 法で強連結成分分解を行う |
| `SCC::build(g)` | 強連結成分分解を行い直す |
| `SCC::same(a, b)` | `a` と `b` が同じ強連結成分なら true |
| `SCC::id(v)` | `v` が属する強連結成分 ID を返す |
| `SCC::size(v)` | `v` が属する強連結成分のサイズを返す |
| `DAGLongestPath<T>` | `dist`, `parent`, `order`, `is_dag` を持つ DAG 最長路結果。`T` のデフォルトは `ll` |
| `DAGLongestPath(g, s)` | `s` から DAG 最長路を計算する |
| `DAGLongestPath(g)` | 全頂点を始点候補として DAG 最長路を計算する |
| `DAGLongestPath::build(g, s)` | `s` から DAG 最長路を計算し直す |
| `DAGLongestPath::build_all(g)` | 全頂点を始点候補として DAG 最長路を計算し直す |
| `DAGLongestPath::distance(t)` | `t` への最長距離を返す |
| `DAGLongestPath::reachable(t)` | `t` に到達可能なら true |
| `DAGLongestPath::path(t)` | 最長路の頂点列を外部 index で返す。不可能なら空 |

`order`, `indeg`, `cycle`, `comp_id`, `groups`, `dist`, `parent` は内部 0-indexed の頂点番号に基づいて保持します。`TopologicalSort` で閉路がある場合、`is_dag` は `false` になり、`order` は処理できた頂点のみを含みます。`DirectedCycle::cycle` は閉路上の頂点列を 1 周分だけ保持し、始点の重複は含みません。`SCC::same`, `SCC::id`, `SCC::size`, `DAGLongestPath` の始点・終点は `Graph::one_indexed` に従う外部 index です。`DAGLongestPath` はグラフが DAG でない場合 `is_dag = false` になり、距離緩和を行いません。

## coloring.hpp

グラフ彩色を提供します。一般グラフの k 色彩色判定は指数時間です。

| Name | Specification |
| --- | --- |
| `KColoring` | `k`, `color`, `found` を持つ k 色彩色結果 |
| `KColoring(g, k)` | DSATUR 順のバックトラックで k 色彩色を試す |
| `KColoring::build(g, k)` | k 色彩色を試し直し、成功したら true |
| `KColoring::min_colors(g)` | 彩色可能な最小色数を小さい順に探索して返す |
| `KColoring::valid()` | 現在の `color` が有効なら true |

`color[v]` は内部 0-indexed 頂点 `v` の色番号を `0 <= color[v] < k` で保持します。彩色では有向辺も隣接制約として扱い、辺が片方向に存在するだけでも両端は異なる色である必要があります。自己ループがあるグラフは彩色不能です。
